...
class TogglRepo constructor(private val dataStore: DataStore) {
    ...
    fun getTogglTimeEntries(date: LocalDate): List<TimeEntry> {
        val apiKey = getTogglApiKey()
        val start = date.atStartOfDay().format(DateTimeFormatter.ISO_DATE)
        val end = date.plusDays(1).atStartOfDay().format(DateTimeFormatter.ISO_DATE)
    ...
    private fun getTogglApiKey(renew: Boolean = false): String {
        ...
        while (key.isNullOrEmpty()) {
            println("🔑 Setup Toggl API Key: Please visit https://track.toggl.com/profile and copy the key from the 'API Token' section here + click Enter:")
            key = readLine()
    ...
    fun pushProjectsToToggl(
        ...
        newProjects.forEachIndexed { index, it ->
            ...
            println("Progress:️ ${index + 1}/${newProjects.size}")
        }
        println("🎉 Your Redmine projects are now available in Toggl!")
    }
